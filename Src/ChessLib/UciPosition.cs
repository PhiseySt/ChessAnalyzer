...
/// </summary>
public class UciPosition : ICloneable
{
    /// <summary>
    /// Create the start position 
    /// </summary>
    public UciPosition() { SetFromFen(UciFen.InitialPosition); }

    /// <summary>
    /// Create a position from a FEN string
    /// </summary>
    /// <param name="fen">The position specified in FEN notation</param>
    public UciPosition(string fen) { if (fen != null) SetFromFen(fen); }
    ...
    /// </summary>
    public bool IsDoubleCheck { get { return Checked() && _checkstate == Checkstate.DoubleChecked; } }
    ...
    /// </summary>
    public bool IsStalemate => GetMoves().Count == 0 && !IsCheck;
    ...
    public void ApplyMove(UciMove move)
    {
        //Console.WriteLine(move.ToUCIString());
        _moves = null;
        _checkstate = Checkstate.Undefined;
        X88Square from = (X88Square)UciX88.X88Index((int)move.From);
        X88Square to = (X88Square)UciX88.X88Index((int)move.To);
        Piece movingPiece = x88board[(int)from];
        DrawPlyCount++;
        //special cases:
        //1. Castles
        if (!Chess960 && UciChess.GetPieceType(x88board[(int)from]) == PieceType.King && Math.Abs(from - to) == 2)
        {
            ApplyStandardCastling(to);
            goto End;
        }
        else if (Chess960 && UciChess.GetPieceType(x88board[(int)from]) == PieceType.King && x88board[(int)to] == UciChess.GetPiece(PieceType.Rook, SideToMove))
        {
            Apply960Castling(to);
            goto End;
        }
        ...
        else if (UciChess.GetPieceType(x88board[(int)from]) == PieceType.Pawn && move.To == EPSquare)
        {
            DrawPlyCount = 0;
            PolyglotKey ^= PolyglotKeys[64 * PolyglotPieceIndex[x88board[(int)from]] + (int)move.From];
        ...
        {
            if (UciChess.GetPieceType(x88board[(int)from]) == PieceType.Pawn || x88board[(int)to] != Piece.Blank) DrawPlyCount = 0;
    ...
    End:
        ...
        if (UciChess.GetPieceType(x88board[(int)to]) == PieceType.Pawn && Math.Abs((int)to - (int)from) == 32)
        {
            //Check if there is an opposite colored pawn next to to field
            if (SideToMove == Side.White)
            {
                if (x88board[(int)to - 1] == Piece.Bpawn || x88board[(int)to + 1] == Piece.Bpawn) EPSquare = (Square)UciX88.Index((int)to - 16);
            }
            else
            {
                if (x88board[(int)to - 1] == Piece.Wpawn || x88board[(int)to + 1] == Piece.Wpawn) EPSquare = (Square)UciX88.Index((int)to + 16);
            }
        }
        if (EPSquare != Square.Outside) PolyglotKey ^= PolyglotKeys[772 + (((int)EPSquare) & 7)];
        if (castlings != 0)
        {
            ...
            if (movingPiece == Piece.Wking) castlings &= (int)CastleFlag.B00 | (int)CastleFlag.B000;
            else if (movingPiece == Piece.Bking) castlings &= (int)CastleFlag.W00 | (int)CastleFlag.W000;
            else
            {
                if (Chess960)
                {
                    if ((int)from == UciX88.X88Index((int)castleableRooks[0]) || (int)to == UciX88.X88Index((int)castleableRooks[0])) castlings &= ~(int)CastleFlag.W00;
                    else if ((int)from == UciX88.X88Index((int)castleableRooks[1]) || (int)to == UciX88.X88Index((int)castleableRooks[1])) castlings &= ~(int)CastleFlag.W000;
                    if ((int)from == UciX88.X88Index((int)castleableRooks[2]) || (int)to == UciX88.X88Index((int)castleableRooks[2])) castlings &= ~(int)CastleFlag.B00;
                    else if ((int)from == UciX88.X88Index((int)castleableRooks[3]) || (int)to == UciX88.X88Index((int)castleableRooks[3])) castlings &= ~(int)CastleFlag.B000;
                }
                else
                {
                    if (from == X88Square.A1 || to == X88Square.A1) castlings &= ~(int)CastleFlag.W000;
                    else if (from == X88Square.H1 || to == X88Square.H1) castlings &= ~(int)CastleFlag.W00;
                    if (from == X88Square.A8 || to == X88Square.A8) castlings &= ~(int)CastleFlag.B000;
                    else if (from == X88Square.H8 || to == X88Square.H8) castlings &= ~(int)CastleFlag.B00;
                }
            }
            PolyglotKey ^= PolyglotCastlingKey(castlings);
        }
    ...
    private void Apply960Castling(X88Square to)
    {
        ...
        Piece king = UciChess.GetPiece(PieceType.King, SideToMove);
        Piece rook = UciChess.GetPiece(PieceType.Rook, SideToMove);
        x88board[(int)from] = x88board[(int)to] = Piece.Blank;
        PolyglotKey ^= PolyglotKeys[64 * PolyglotPieceIndex[king] + UciX88.Index(from)];
        PolyglotKey ^= PolyglotKeys[64 * PolyglotPieceIndex[rook] + UciX88.Index(to)];
        PolyglotKey ^= PolyglotCastlingKey(castlings);
        switch (index)
        {
            case 0:
                ...
                castlings &= (int)CastleFlag.B00 | (int)CastleFlag.B000;
            ...
            case 2:
                ...
                castlings &= (int)CastleFlag.W00 | (int)CastleFlag.W000;
    ...
    private void ApplyStandardCastling(X88Square to)
    {
        ...
        {
            x88board[(int)X88Square.E1] = x88board[(int)X88Square.A1] = Piece.Bking;
            x88board[(int)X88Square.C1] = Piece.Wking;
            x88board[(int)X88Square.D1] = Piece.Wrook;
            PolyglotKey ^= PolyglotKeys[64 * PolyglotPieceIndex[Piece.Wking] + (int)Square.E1];
            PolyglotKey ^= PolyglotKeys[64 * PolyglotPieceIndex[Piece.Wking] + (int)Square.C1];
    ...
    internal bool UndoMove(UciExtendedMove move, ulong polyglotKeyBefore)
    {
        ...
        PieceType pt = UciChess.GetPieceType(p);
        X88Square from = (X88Square)UciX88.X88Index((int)move.From);
        X88Square to = (X88Square)UciX88.X88Index((int)move.To);
    ...
    {
        if (CastlingAllowed(CastleFlag.W00) && (GetPiece(Square.E1) != Piece.Wking || GetPiece(Square.H1) != Piece.Wrook)) return false;
        if (CastlingAllowed(CastleFlag.W000) && (GetPiece(Square.E1) != Piece.Wking || GetPiece(Square.A1) != Piece.Wrook)) return false;
        if (CastlingAllowed(CastleFlag.B00) && (GetPiece(Square.E8) != Piece.Bking || GetPiece(Square.H8) != Piece.Brook)) return false;
        if (CastlingAllowed(CastleFlag.B000) && (GetPiece(Square.E8) != Piece.Bking || GetPiece(Square.A8) != Piece.Brook)) return false;
    ...
    {
        ApplyMove(new UciMove(moveInUciNotation));
    }
    ...
    public Piece GetPiece(Square square) { return x88board[UciX88.X88Index((int)square)]; }
    ...
    public List<UciMove> GetMoves()
    {
        ...
        return new List<UciMove>(_moves);
    }
    ...
    internal string ToLan(UciMove move, IChessPieceStringProvider icp)
    {
        StringBuilder sb = new();
        PieceType pt = UciChess.GetPieceType(x88board[UciX88.X88Index((int)move.From)]);
        ...
        else if (pt == PieceType.King && castlings != 0 && Chess960 && GetPiece(move.To) != Piece.Blank && ((int)GetPiece(move.To) & 1) == (int)SideToMove)
        {
            if (SideToMove == Side.White)
            {
                if (((UciMove)move).Equals(CastleMove(CastleFlag.W00))) sb.Append("O-O");
                else if (((UciMove)move).Equals(CastleMove(CastleFlag.W000))) sb.Append("O-O-O");
            }
            else
            {
                if (((UciMove)move).Equals(CastleMove(CastleFlag.B00))) sb.Append("O-O");
                else if (((UciMove)move).Equals(CastleMove(CastleFlag.B000))) sb.Append("O-O-O");
            }
        }
        else
        {
            bool capture = x88board[UciX88.X88Index((int)move.To)] != Piece.Blank
                || (pt == PieceType.Pawn && move.To == EPSquare);
            if (pt != PieceType.Pawn) sb.Append(icp.Get(pt));
            sb.Append(UciChess.SquareToString(move.From));
            if (capture) sb.Append('x'); else sb.Append('-');
            sb.Append(UciChess.SquareToString(move.To));
            if (move.PromoteTo != PieceType.None) sb.Append('=').Append(icp.Get(move.PromoteTo));
        }
        
        var next = (UciPosition) Clone();
        next.ApplyMove(move);
        if (next.IsMate) sb.Append('#'); else if (next.IsCheck) sb.Append('+');
        return sb.ToString();
    }

    internal string ToSan(UciMove move, IChessPieceStringProvider icp)
    {
        StringBuilder sb = new();
        PieceType pt = UciChess.GetPieceType(x88board[UciX88.X88Index((int)move.From)]);
        ...
        else
        {
            ...
            foreach (UciMove m in GetMoves())
            {
                if (m.To == move.To && m.From != move.From && pt == UciChess.GetPieceType(x88board[UciX88.X88Index((int)m.From)]))
                {
        ...
        UciPosition next = (UciPosition)Clone();
        next.ApplyMove(move);
        if (next.IsMate) sb.Append('#'); else if (next.IsCheck) sb.Append('+');
        return sb.ToString();
    }

    internal static string ToIccf(UciMove move)
    {
        StringBuilder sb = new();
    ...
    public string ToSAN(UciMove move)
    {
        return ToSan(move, UciChess.PieceChars[CultureInfo.InvariantCulture]);
    }
    ...
    public bool CheckLegal(out string? message)
    {
        ...
        {
            if (SideToMove == Side.White && EPSquare < Square.A5) message = $"Invalid EPSquare {UciChess.SquareToString(EPSquare)}";
            if (SideToMove == Side.Black && EPSquare > Square.H4) message = $"Invalid EPSquare {UciChess.SquareToString(EPSquare)}";
            if ((EPSquare < Square.A3 || EPSquare > Square.H3) && (EPSquare < Square.A6 || EPSquare > Square.H6)) message = $"Invalid EPSquare {UciChess.SquareToString(EPSquare)}";
            if (GetPiece(EPSquare) != Piece.Blank) message = $"EnPassant Square {UciChess.SquareToString(EPSquare)} is not empty!";
            if (EPSquare >= Square.A6)
            {
                Square s = (Square)((int)EPSquare - 8);
                if (GetPiece(s) != Piece.Bpawn) message = $"Missing EnPassant target on {UciChess.SquareToString(s)}!";
            }
            else if (EPSquare >= Square.A3)
            {
                Square s = (Square)((int)EPSquare + 8);
                if (GetPiece(s) != Piece.Wpawn) message = $"Missing EnPassant target on {UciChess.SquareToString(s)}!";
            }
        }
        return message == null;
    ...
    public long Perft(int depth, bool divide = false)
    {
        ...
        long result = 0;
        foreach (UciMove move in GetMoves())
        {
            var next = (UciPosition)Clone();
            next.ApplyMove(move);
            if (divide)
    ...
    public Dictionary<UciMove, long> Divide(int depth)
    {
        Dictionary<UciMove, long> result = new();
        foreach (UciMove move in GetMoves())
        {
            var next = (UciPosition)Clone();
            next.ApplyMove(move);
    ...
    private enum Checkstate { Undefined, NoCheck, Checked, DoubleChecked };

    private readonly Piece[] x88board = new Piece[120];
    internal int castlings = 0;
    private List<UciMove>? _moves;
    ...
    private void SetFromFen(string fen)
    {
        ...
        for (int rank = 7; rank >= 0; --rank)
        {
            ...
            foreach (char c in ranks[7 - rank])
            {
                ...
                {
                    Piece p = UciFen.PieceMapping[c];
                    x88board[UciX88.X88Index(rank, file)] = UciFen.PieceMapping[c];
                    PolyglotKey ^= PolyglotKeys[64 * PolyglotPieceIndex[p] + 8 * rank + file];
                    material[(int)p]++;
                    if (p == Piece.Wking) kingSquares[0] = (X88Square)UciX88.X88Index(rank, file);
                    else if (p == Piece.Bking) kingSquares[1] = (X88Square)UciX88.X88Index(rank, file);
                    file++;
                }
            }
        }
        ...
        if (token[2] != "-")
        {
            ...
            if (!Chess960)
            {
                foreach (char c in token[2]) castlings |= (int)UciFen.CastleFlagMapping[c];
            }
            PolyglotKey ^= PolyglotCastlingKey(castlings);
        }
        ...
        {
            Square tempEPSquare = UciFen.ParseSquare(token[3]);
            //Check if there is a pawn available for capturing
            int x88Square = UciX88.X88Index((int)tempEPSquare);
            if (SideToMove == Side.White)
            {
                if (UciX88.IsOnBoard(x88Square - 17) && x88board[x88Square - 17] == Piece.Wpawn)
                {
                    EPSquare = tempEPSquare;
                }
                else if (UciX88.IsOnBoard(x88Square - 15) && x88board[x88Square - 15] == Piece.Wpawn)
                {
                    EPSquare = tempEPSquare;
                }
            }
            ...
            {
                if (UciX88.IsOnBoard(x88Square + 17) && x88board[x88Square + 17] == Piece.Bpawn)
                {
                    EPSquare = tempEPSquare;
                }
                else if (UciX88.IsOnBoard(x88Square + 15) && x88board[x88Square + 15] == Piece.Bpawn)
                {
                    EPSquare = tempEPSquare;
                }
            }
    ...
    private void Check960(string v)
    {
        ...
        if (is960)
        {
            ...
            foreach (char c in v)
            {
                ...
                {
                    case 'K':
                        castlings |= (int)CastleFlag.W00;
                        foreach (Square sq in psql[Piece.Wrook])
                        {
                            if (sq <= Square.H1 && (int)sq > UciX88.Index(kingSquares[0]))
                            {
                        ...
                        break;
                    case 'Q':
                        castlings |= (int)CastleFlag.W000;
                        foreach (Square sq in psql[Piece.Wrook])
                        {
                            if ((int)sq < UciX88.Index(kingSquares[0]))
                            {
                        ...
                        break;
                    case 'k':
                        castlings |= (int)CastleFlag.B00;
                        foreach (Square sq in psql[Piece.Brook])
                        {
                            if (sq <= Square.H8 && (int)sq > UciX88.Index(kingSquares[1]))
                            {
                        ...
                        break;
                    case 'q':
                        castlings |= (int)CastleFlag.B000;
                        foreach (Square sq in psql[Piece.Brook])
                        {
                            if (sq >= Square.A8 && (int)sq < UciX88.Index(kingSquares[1]))
                            {
                    ...
                    default:
                        if (Char.IsUpper(c))
                        {
                            ...
                            int s = file;
                            if (s > UciX88.Index(kingSquares[0]))
                            {
                        ...
                        else
                        {
                            int file = (int)(c) - (int)'a';
                            int s = file + 56;
                            if (s > UciX88.Index(kingSquares[1]))
                            {
    ...
    private string ToFen(bool fullFEN = true, bool sFen = false)
    {
        ...
        for (int rank = 7; rank >= 0; --rank)
        {
            ...
            while (file < 8)
            {
                Piece p = x88board[UciX88.X88Index(rank, file)];
                if (p == Piece.Blank)
                {
                    ++count;
        ...
        {
            if (CastlingAllowed(CastleFlag.W00)) sb.Append("ABCDEFGH"[(int)castleableRooks[0] & 7]);
            if (CastlingAllowed(CastleFlag.W000)) sb.Append("ABCDEFGH"[(int)castleableRooks[1] & 7]);
            if (CastlingAllowed(CastleFlag.B00)) sb.Append("abcdefgh"[(int)castleableRooks[2] & 7]);
            if (CastlingAllowed(CastleFlag.B000)) sb.Append("abcdefgh"[(int)castleableRooks[3] & 7]);
        }
        else
        {
            if (Chess960)
            {
                ...
                if (CastlingAllowed(CastleFlag.W00))
                {
                ...
                if (CastlingAllowed(CastleFlag.W000))
                {
                ...
                if (CastlingAllowed(CastleFlag.B00))
                {
                ...
                if (CastlingAllowed(CastleFlag.B000))
                {
            ...
            {
                if (CastlingAllowed(CastleFlag.W00)) sb.Append('K');
                if (CastlingAllowed(CastleFlag.W000)) sb.Append('Q');
                if (CastlingAllowed(CastleFlag.B00)) sb.Append('k');
                if (CastlingAllowed(CastleFlag.B000)) sb.Append('q');
            }
        }
        sb.Append(' ');
    ...
    private void GenerateMoves()
    {
        if (_moves == null)
        {
            ...
            else
            {
                ...
                {
                    if (x88board[square] == Piece.Blank || UciChess.GetColor(x88board[square]) != SideToMove || !UciX88.IsOnBoard(square)) continue;
                    PieceType movingPT = UciChess.GetPieceType(x88board[square]);
                    switch (movingPT)
                    {
                        case PieceType.King:
    ...
    {
        int from = UciX88.Index(square);
        sbyte pinDirection = IsPinned((X88Square)square);
        foreach (sbyte step in UciX88.RookDirections)
        {
            for (int i = 1; i < 8; ++i)
            {
                int to = square + i * step;
                if (!UciX88.IsOnBoard(to)) break;
                if (x88board[to] == Piece.Blank)
                {
                    if (pinDirection == 0 && !IsCheck) _moves?.Add(new UciMove(from, UciX88.Index(to)));
                    else AddIfLegal(new UciMove(from, UciX88.Index(to)));
                }
                else
                {
                    if (UciChess.GetColor(x88board[(int)to]) != SideToMove)
                    {
                        if (pinDirection == 0 && !IsCheck) _moves?.Add(new UciMove(from, UciX88.Index(to)));
                        else AddIfLegal(new UciMove(from, UciX88.Index(to)));
                    }
    ...
    private void GenerateMovesBishop(int square)
    {
        int from = UciX88.Index(square);
        sbyte pinDirection = IsPinned((X88Square)square);
        foreach (sbyte step in UciX88.BishopDirections)
        {
            for (int i = 1; i < 8; ++i)
            {
                int to = square + i * step;
                if (!UciX88.IsOnBoard((int)to)) break;
                if (x88board[to] == Piece.Blank)
                {
                    if (pinDirection == 0 && !IsCheck) _moves?.Add(new UciMove(from, UciX88.Index(to)));
                    else AddIfLegal(new UciMove(from, UciX88.Index(to)));
    ...
    private void GenerateMovesPawn(int square)
    {
        ...
        if (SideToMove == Side.White)
        {
            ...
            {
                int to = UciX88.Index(square + 16);
                if (square >= (int)X88Square.A7)
                {
                    if (pinDirection == 0 && !IsCheck)
                    {
                        _moves?.Add(new UciMove(from, to, PieceType.Queen));
                        _moves?.Add(new UciMove(from, to, PieceType.Rook));
                        _moves?.Add(new UciMove(from, to, PieceType.Bishop));
                        _moves?.Add(new UciMove(from, to, PieceType.Knight));
                    }
                    else
                    {
                        AddIfLegal(new UciMove(from, to, PieceType.Queen));
                        AddIfLegal(new UciMove(from, to, PieceType.Rook));
                        AddIfLegal(new UciMove(from, to, PieceType.Bishop));
                        AddIfLegal(new UciMove(from, to, PieceType.Knight));
                    }
                }
                else
                {
                    if (pinDirection == 0 && !IsCheck)
                    {
                        _moves?.Add(new UciMove(from, to));
                        if (square <= (int)X88Square.H2 && x88board[square + 32] == Piece.Blank)
                            _moves?.Add(new UciMove(from, UciX88.Index(square + 32)));
                    }
                    else
                    {
                        AddIfLegal(new UciMove(from, to));
                        if (square <= (int)X88Square.H2 && x88board[square + 32] == Piece.Blank)
                            AddIfLegal(new UciMove(from, UciX88.Index(square + 32)));
                    }
                }
            }
            int[] steps = { 15, 17 };
            foreach (int step in steps)
            {
                ...
                if (!UciX88.IsOnBoard((int)toSquare)) continue;
                int to = UciX88.Index(square + step);
                if (x88board[(int)toSquare] != Piece.Blank && UciChess.GetColor(x88board[(int)toSquare]) == Side.Black)
                {
                    if (square >= (int)X88Square.A7)
                    {
                        if (pinDirection == 0 && !IsCheck)
                    ...
                    {
                        if (pinDirection == 0 && !IsCheck) _moves?.Add(new UciMove(from, to));
                        else AddIfLegal(new UciMove(from, to));
                    }
                }
                if (to == (int)EPSquare)
                {
        ...
        else
        {
            ...
            foreach (int step in steps)
            {
                ...
                if (!UciX88.IsOnBoard((int)toSquare)) continue;
                int to = UciX88.Index(square + step);
                if (x88board[(int)toSquare] != Piece.Blank && UciChess.GetColor(x88board[(int)toSquare]) == Side.White)
                {
                    if (square <= (int)X88Square.H2)
                    {
                        if (pinDirection == 0 && !IsCheck)
    ...
    {
        int from = UciX88.Index(square);
        foreach (sbyte step in UciX88.KingDirections)
        {
            int toSquare = square + step;
            if (UciX88.IsOnBoard(toSquare) && (x88board[toSquare] == Piece.Blank || UciChess.GetColor(x88board[toSquare]) != SideToMove))
            {
                AddIfLegal(new UciMove(from, UciX88.Index(toSquare)));
            }
        }
        if (Checked()) return;
        ...
        else
        {
            ...
            {
                if ((castlings & (int)CastleFlag.W00) != 0 && CheckEmptyInBetween(Square.E1, Square.H1))
                {
                    AddIfLegal(UciMove.W00, true);
                }
                if ((castlings & (int)CastleFlag.W000) != 0 && CheckEmptyInBetween(Square.A1, Square.E1))
                {
                    AddIfLegal(UciMove.W000, true);
                }
            }
            else if (SideToMove == Side.Black && (castlings & 12) != 0)
            {
                if ((castlings & (int)CastleFlag.B00) != 0 && CheckEmptyInBetween(Square.E8, Square.H8))
                {
                    AddIfLegal(UciMove.B00, true);
                }
                if ((castlings & (int)CastleFlag.B000) != 0 && CheckEmptyInBetween(Square.A8, Square.E8))
                {
                    AddIfLegal(UciMove.B000, true);
                }
            }
        }
    }
    ...
    {
        Square square = (Square)UciX88.Index(x88square);
        if (SideToMove == Side.White && (castlings & 3) != 0)
        {
            if ((castlings & (int)CastleFlag.W00) != 0 && CheckEmptyInBetween(square, Square.G1, castleableRooks[0]) && CheckEmptyInBetween(castleableRooks[0], Square.F1, square) &&
                ...
                AddIfLegal(CastleMove(CastleFlag.W00), true);
            if ((castlings & (int)CastleFlag.W000) != 0 && CheckEmptyInBetween(square, Square.C1, castleableRooks[1]) && CheckEmptyInBetween(castleableRooks[1], Square.D1, square) &&
                ...
                AddIfLegal(CastleMove(CastleFlag.W000), true);
        }
        else if (SideToMove == Side.Black && (castlings & 12) != 0)
        {
            if ((castlings & (int)CastleFlag.B00) != 0 && CheckEmptyInBetween(square, Square.G8, castleableRooks[2]) && CheckEmptyInBetween(castleableRooks[2], Square.F8, square) &&
                ...
                AddIfLegal(CastleMove(CastleFlag.B00), true);
            if ((castlings & (int)CastleFlag.B000) != 0 && CheckEmptyInBetween(square, Square.C8, castleableRooks[3]) && CheckEmptyInBetween(castleableRooks[3], Square.D8, square) &&
                ...
                AddIfLegal(CastleMove(CastleFlag.B000), true);
        }
    }
    ...
    private void GenerateMovesKnight(int square)
    {
        int from = UciX88.Index(square);
        sbyte pinDirection = IsPinned((X88Square)square);
        foreach (sbyte step in UciX88.KnightMoves)
        {
            int toSquare = square + step;
            if (UciX88.IsOnBoard(toSquare) && (x88board[toSquare] == Piece.Blank || UciChess.GetColor(x88board[toSquare]) != SideToMove))
            {
                if (pinDirection == 0 && !IsCheck) _moves?.Add(new UciMove(from, UciX88.Index(toSquare)));
                else AddIfLegal(new UciMove(from, UciX88.Index(toSquare)));
            }
        }
    }

    private bool AddIfLegal(UciMove move, bool isCastling = false)
    {
    ...
    private bool CheckLegal(UciMove move, bool isCastling = false)
    {
        if (!isCastling)
        {
            var next = (UciPosition)Clone();
        ...
        return targets != null && targets.All(square => !IsAttacked(square, other));
    }

    private bool CheckLegal960(UciMove move)
    {
        ...
        if (target > move.From)
        {
            for (int i = (int)move.From; i <= (int)target; ++i) targets.Add((X88Square)UciX88.X88Index(i));
        }
        else
        {
            for (int i = (int)target; i <= (int)move.From; ++i) targets.Add((X88Square)UciX88.X88Index(i));
        }
        Side other = (Side)((int)SideToMove ^ 1);
        ...
        var next = (UciPosition)Clone();
        next.ApplyMove(move);
        if (next.IsAttacked((X88Square)UciX88.X88Index((int)target), other)) return false;
        return true;
    }

    internal UciMove CastleMove(CastleFlag cf)
    {
        return cf switch
        {
            CastleFlag.W00 => Chess960 ? new UciMove((Square)UciX88.Index(kingSquares[0]), castleableRooks[0]) : UciMove.W00,
            CastleFlag.W000 => Chess960 ? new UciMove((Square)UciX88.Index(kingSquares[0]), castleableRooks[1]) : UciMove.W000,
            CastleFlag.B00 => Chess960 ? new UciMove((Square)UciX88.Index(kingSquares[1]), castleableRooks[2]) : UciMove.B00,
            CastleFlag.B000 => Chess960 ? new UciMove((Square)UciX88.Index(kingSquares[1]), castleableRooks[3]) : UciMove.B000,
            _ => UciMove.Null
        };
    }
    ...
    private bool Checked()
    {
        ...
        {
            _checkstate = Checkstate.NoCheck;
            for (int i = 0; i < 64; ++i)
            {
                X88Square square = (X88Square)UciX88.X88Index(i);
                Piece p = x88board[(int)square];
                if (p >= Piece.Wking || UciChess.GetColor(p) == SideToMove) continue;
                if (UciX88.IsAttacking(p, square, kingSquares[(int)SideToMove]))
                {
                    PieceType pt = UciChess.GetPieceType(p);
                    if (pt == PieceType.Knight || pt == PieceType.Pawn)
                    {
                        ++_checkstate;
                        if (_checkstate == Checkstate.DoubleChecked)
                            return true;
                    }
                    else
                    {
                        //slider
                        sbyte direction = UciX88.GetDirection(square, kingSquares[(int)SideToMove]);
                        bool blocked = false;
    ...
    private bool IsAttacked(X88Square targetSquare, Side attacking)
    {
        for (int i = 0; i < 64; ++i)
        {
            X88Square square = (X88Square)UciX88.X88Index(i);
            Piece p = x88board[(int)square];
            if (p == Piece.Blank || UciChess.GetColor(p) != attacking || square == targetSquare) continue;
            if (UciX88.IsAttacking(p, square, targetSquare))
            {
                PieceType pt = UciChess.GetPieceType(p);
                ...
                {
                    //slider
                    sbyte direction = UciX88.GetDirection(square, targetSquare);
                    bool blocked = false;
                    int checkSquare = (int)square + direction;
    ...
    {
        sbyte dir = UciX88.GetDirection(kingSquares[(int)SideToMove], square);
        if (dir != 0)
        {
            int s = (int)square + dir;
            while (UciX88.IsOnBoard(s))
            {
                Piece piece = x88board[s];
                if (piece == Piece.Blank) s += dir;
                else
                {
                    if (UciChess.GetColor(piece) == SideToMove) return 0;
                    else
                    {
                        if (UciX88.IsAttacking(piece, (X88Square)s, kingSquares[(int)SideToMove])) return dir; else return 0;
                    }
                }
            }
        }
    ...
    {
        if (CastlingAllowed(CastleFlag.W00) && (GetPiece(Square.E1) != Piece.Wking || GetPiece(Square.H1) != Piece.Wrook)) castlings &= ~(int)CastleFlag.W00;
        if (CastlingAllowed(CastleFlag.W000) && (GetPiece(Square.E1) != Piece.Wking || GetPiece(Square.A1) != Piece.Wrook)) castlings &= ~(int)CastleFlag.W000;
        if (CastlingAllowed(CastleFlag.B00) && (GetPiece(Square.E8) != Piece.Bking || GetPiece(Square.H8) != Piece.Brook)) castlings &= ~(int)CastleFlag.B00;
        if (CastlingAllowed(CastleFlag.B000) && (GetPiece(Square.E8) != Piece.Bking || GetPiece(Square.A8) != Piece.Brook)) castlings &= ~(int)CastleFlag.B000;
    }
}