...
public class UciX88
{
    internal static sbyte[] KnightMoves = { -33, -31, -18, -14, +14, +18, +31, +33 };
    internal static sbyte[] KingDirections = { 17, 16, 15, 1, -1, -15, -16, -17 };
    internal static sbyte[] RookDirections = { 16, 1, -1, -16 };
    internal static sbyte[] BishopDirections = { 17, 15, -17, -15 };
    ...
    static UciX88()
    {
        ...
        sbyte[] dir = { 16, -16, 1, -1, 17, -17, 15, -15 };
        for (var from = Square.A1; from <= Square.H8; ++from)
        {
            var xfrom = (X88Square)X88Index((int)from);
            foreach (var d in dir)
            {
                for (var xto = (int)xfrom + d; xto < 120; xto += d)
                {
        ...
        for (var from = Square.A1; from <= Square.H8; ++from)
        {
            var xfrom = (X88Square)X88Index((int)from);
            //King attacks
            foreach (var d in KingDirections)
            {
                var xto = (int)xfrom + d;
                if (IsOnBoard(xto))
                {
                    var indx = 0x77 + (int)xfrom - xto;
                    attacks[indx] |= 1 << (int)Piece.Wking;
                    attacks[indx] |= 1 << (int)Piece.Bking;
                }
            }
            //Knight attacks
            foreach (var d in KnightMoves)
            {
                var xto = (int)xfrom + d;
                if (IsOnBoard(xto))
                {
            ...
            //Rooks
            foreach (var d in RookDirections)
            {
                for (var n = 1; n <= 8; ++n)
                {
                    var xto = (int)xfrom + n * d;
                    if (IsOnBoard(xto))
                    {
                        var indx = 0x77 + (int)xfrom - xto;
